...
public final class Mapping<S, D> {
  ...
  /**
   * Maps a property from the source to the specified property of the destination object with transforming the source
   * value using the specified transform lambda. <b>Note: The mapping library is designed to reduce the required client
   * tests. Using this method requires the client to test the transformation function!</b>
   *
   * @param sourceSelector The {@link FieldSelector}s selecting the source property with get-method invocation.
  ...
  /**
   * This method adds a strict mapping for all unmapped properties of source that have a corresponding property in the
   * destination type.
   */
  private void addStrictMapping() {
    ...
    getUnmappedProperties(source, mappedSourceProperties).stream()
            ...
            .forEach(pd -> {
              ...
              // transformation
              PropertyDescriptor destinationProperty = getPropertyDescriptorByPropertyName(
                      unmappedDestinationProperties, pd.getName());
  ...
  private PropertyDescriptor getPropertyDescriptorByPropertyName(Set<PropertyDescriptor> descriptors,
    ...
    if (matchedPropertiesByName.isEmpty() || matchedPropertiesByName.size() > 1) {
      throw new MappingException(
              String.format("Cannot assign source property '%s' to destination, but this was determined to be possible"
  ...
  /**
   * Returns all properties from the specified type, that were unmapped in the specified {@link Set} of {@link
  ...
  /**
   * Executes a {@link FieldSelector} lambda on a proxy object of the specified type and returns the {@link
  ...
  /**
   * Registers a configured mapper to this object that is to be used whenever a hierarchical mapping tries to map the
   * specified types. <b>Note: Only one mapper can be added for a combination of source and destination type!</b>
   *
  ...
  /**
   * Returns a registered mapper for hierarchical mapping. If the desired mapper was not found a {@link
   * MappingException} is thrown.
   *
